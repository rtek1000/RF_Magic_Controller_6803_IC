...
Output data appears to be being sent at an incredible 2.4 Mbps. The signal seems to be inverted, with idle logic at 0V (low state), different from the display communication which has high state idle logic. Data capture (display and output control) are in the Logic folder. Note: The output data is only from the control module, it was not connected with the LED strip.
